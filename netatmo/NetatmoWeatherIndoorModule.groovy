...
metadata {
    definition(
        name: "Netatmo Weather Indoor Module",
        namespace: "brossow",
        author: "Brent Rossow",
        description: "Netatmo Weather Station additional indoor module"
    ) {
        capability "Sensor"
        capability "Refresh"
        capability "Battery"
...
def installed() {
    log.info "Netatmo Weather Indoor Module installed"
}

def updated() {
    log.info "Netatmo Weather Indoor Module updated"
}
...
def refresh() {
    try {
        if (parent) {
            ...
            log.warn "Netatmo Weather Indoor Module refresh skipped: parent app is unavailable"
        }
    } catch (Exception e) {
        log.warn "Netatmo Weather Indoor Module refresh failed: ${e?.message ?: e}"
    }
}

def updatedFromParent(Map data) {
    if (!(data instanceof Map)) {
        log.warn "Netatmo Weather Indoor Module update skipped: parent data was missing or invalid"
        return
    }
