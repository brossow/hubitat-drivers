...
<script>(function(){try{var d=new Date('${updated}');if(isNaN(d.getTime()))return;var n=new Date(),same=d.toDateString()===n.toDateString(),fmt={month:'short',day:'numeric'};document.getElementById('ts').textContent=same?d.toLocaleTimeString():d.toLocaleDateString(undefined,fmt)+' '+d.toLocaleTimeString();}catch(e){}})();(function(){var g=document.querySelector('.ams-body');if(!g||g.children.length<2||g.style.gridTemplateColumns)return;var n=g.children.length,gw=parseFloat(getComputedStyle(g).columnGap)||10,max=Math.min(n,Math.max(1,Math.floor((g.offsetWidth+gw)/(140+gw)))),best=1,bs=n;for(var c=1;c<=max;c++){var s=n%c===0?0:c-n%c;if(s<bs||(s===bs&&c>best)){bs=s;best=c;}}g.style.gridTemplateColumns='repeat('+best+',1fr)';})();</script>
</body></html>"""
}
