...
metadata {
    definition(
        name: "Netatmo Weather Rain Gauge",
        namespace: "brossow",
        author: "Brent Rossow",
        description: "Netatmo Weather Station rain gauge"
    ) {
        capability "Sensor"
        capability "Refresh"
        capability "Battery"
...
def installed() {
    log.info "Netatmo Weather Rain Gauge installed"
}

def updated() {
    log.info "Netatmo Weather Rain Gauge updated"
}
...
def refresh() {
    try {
        if (parent) {
            ...
            log.warn "Netatmo Weather Rain Gauge refresh skipped: parent app is unavailable"
        }
    } catch (Exception e) {
        log.warn "Netatmo Weather Rain Gauge refresh failed: ${e?.message ?: e}"
    }
}

def updatedFromParent(Map data) {
    if (!(data instanceof Map)) {
        log.warn "Netatmo Weather Rain Gauge update skipped: parent data was missing or invalid"
        return
    }
    ...
    sendEventIfPresent("rain", dashboard.rain, units.rain ?: "mm")
    sendEventIfPresent("rainLastHour", dashboard.rainLastHour, units.rain ?: "mm")
